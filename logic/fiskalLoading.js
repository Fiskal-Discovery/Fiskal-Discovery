function showFiskalLoadingScreen(firstName, onComplete) {
  ...
  const cx = W/2, cy = H/2;
  let offset = 0, time = 0;
  const palette = [[78,0,99],[100,5,130],[126,9,165],[155,48,192],[100,5,130],[78,0,99],[60,20,160],[78,0,99],[126,9,165],[200,70,10],[126,9,165],[78,0,99]];
  function getColor(ringIndex, totalRings) {
    const t = (time*0.003 + ringIndex/totalRings)%1;
    const scaled = t*(palette.length-1);
    const idx = Math.floor(scaled); const frac = scaled-idx;
    const a = palette[idx%palette.length]; const b = palette[(idx+1)%palette.length];
    return [Math.round(a[0]+(b[0]-a[0])*frac),Math.round(a[1]+(b[1]-a[1])*frac),Math.round(a[2]+(b[2]-a[2])*frac)];
  }
  let animFrame;
  function draw() {
    ctx.fillStyle='rgba(5,0,8,0.25)'; ctx.fillRect(0,0,W,H);
    const RINGS=28;
    for(let i=RINGS;i>=0;i--){
      const progress=((i/RINGS)+offset)%1; const size=progress;
      const rx=size*W*0.54; const ry=size*H*0.50;
      const alpha=(1-size)*0.8+0.03; const lw=(1-size)*4+0.3;
    ...
    cg.addColorStop(0,'rgba(180,80,255,0.5)'); cg.addColorStop(0.4,'rgba(126,9,165,0.25)');
    cg.addColorStop(0.8,'rgba(78,0,99,0.1)'); cg.addColorStop(1,'rgba(0,0,0,0)');
    ctx.beginPath(); ctx.arc(cx,cy,75,0,Math.PI*2); ctx.fillStyle=cg; ctx.fill();
    offset+=0.004; time++; animFrame=requestAnimationFrame(draw);
  }
  draw();
  ...
  function showMessage(index){msgEl.style.opacity='0';setTimeout(()=>{msgEl.textContent=messages[index];msgEl.style.opacity='1';},400);}
  showMessage(0);
